...
- 키오스크 앱 (무인 주문기 등):
  ...
  * 메뉴 선택 시 뜨는 옵션 모달(온도, 사이즈, 수량)이 보이면, 옵션 영역과 "장바구니 담기" 버튼을 모두 targets에 포함하여 state=SELECT_OPTION으로 응답하세요. tts는 "온도와 사이즈를 선택하시고, 다 선택하셨으면 장바구니 담기 버튼을 눌러주세요." 형태로 작성하세요. targets의 순서는 [옵션 영역, 장바구니 담기 버튼] 순으로 배치하세요.
