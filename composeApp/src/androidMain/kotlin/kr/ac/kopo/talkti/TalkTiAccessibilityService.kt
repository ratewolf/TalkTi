...
class TalkTiAccessibilityService : AccessibilityService() {
    ...
    private fun openAppByName(appNameOrPackage: String, searchQuery: String? = null): Boolean {
        val pm = packageManager
        Log.d(TAG, "openAppByName 호출: $appNameOrPackage, searchQuery: $searchQuery")

        // 1. 패키지명으로 직접 실행 시도 (LLM이 패키지명을 보낸 경우)
        try {
            if ((appNameOrPackage == "net.daum.android.map" || appNameOrPackage == "com.nhn.android.nmap") && !searchQuery.isNullOrBlank()) {
        ...
        for ((alias, packageList) in aliasMap) {
            if (cleanCmd.contains(alias) || alias.contains(cleanCmd)) {
                ...
                if (installedPackage != null) {
                    if ((installedPackage == "net.daum.android.map" || installedPackage == "com.nhn.android.nmap") && !searchQuery.isNullOrBlank()) {
        ...
        for (appInfo in packages) {
            ...
            if (cleanLabel.length >= 2 && (cleanCmd.contains(cleanLabel) || cleanLabel.contains(cleanCmd))) {
                ...
                if (intent != null) {
                    if ((appInfo.packageName == "net.daum.android.map" || appInfo.packageName == "com.nhn.android.nmap") && !searchQuery.isNullOrBlank()) {
        ...
        return try {
            val uri = when (packageName) {
                "net.daum.android.map" -> {
                    android.net.Uri.parse("kakaomap://search?q=" + android.net.Uri.encode(query))
                }
                "com.nhn.android.nmap" -> {
                    android.net.Uri.parse("nmap://search?query=" + android.net.Uri.encode(query))
                }
                else -> null
            }
            if (uri != null) {
    ...
    private fun processLocalCommand(command: String): Boolean {
        ...
        return openAppByName(cleanCmd, cleanSearchQuery(command))
    }
