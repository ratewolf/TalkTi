...
object PromptTemplates {
    ...
    fun buildScreenAnalyzePrompt(currentIntent: String, historyText: String, uiElementsJson: String, personalizationInfo: String = ""): String {
        val personalizationBlock = if (personalizationInfo.isNotBlank()) "$personalizationInfo\n" else ""
        return """
            $SCREEN_ANALYZE_SYSTEM_PROMPT
            
            $personalizationBlock
            현재 사용자의 최종 목적은 '$currentIntent'입니다. 
            
            [이전 대화 내역 (참조)]
            $historyText
            
            --- 여기서부터가 현재 화면에 대한 데이터입니다 ---
            현재 화면 UI 요소 JSON: $uiElementsJson
        """.trimIndent()
    }
}
