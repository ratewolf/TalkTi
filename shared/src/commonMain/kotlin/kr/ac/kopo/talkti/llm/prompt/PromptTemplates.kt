...
object PromptTemplates {
    ...
    const val SCREEN_ANALYZE_SYSTEM_PROMPT = """
        ...
        2. [범용 리스트 선택 가이드 (경로/장소 목록 포함 - 매우 중요!)] 
           - 화면에 여러 항목(예: 장소 검색 결과 목록, 대중교통 경로 추천 리스트, 자동차 추천 경로 리스트 등)이 나열된 상태라면, 절대 임의로 특정 항목을 직접 선택(CLICK)하거나 성급하게 최종 '안내시작' 혹은 '안내' 버튼을 누르라고 지시하지 마세요. 
           - 특히, 화면 뷰포트나 스크롤 위치 때문에 '안내시작' 버튼이 1개만 화면에 보일지라도, 이 화면의 목적이 "경로 중 하나를 선택하는 화면"이라면 성급한 CLICK 액션을 자제하고, 리스트 컨테이너 전체에 GUIDE 액션을 내리세요.
           - ttsMessage는 어르신이 목록 중에서 직접 비교하고 고를 수 있게 상황에 맞추어 가이드하는 말("어르신, 아래 추천 경로 중 마음에 드는 경로를 눌러주세요.")을 자연스럽게 제공하세요.
        ...
        - 장소 검색 결과 화면이나 경로 목록 화면에서 여러 항목이 있는 경우, 목록 가이드(actionType: GUIDE)를 통해 사용자가 원하는 장소나 경로를 직접 고를 수 있게 하세요.
