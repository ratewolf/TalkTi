...
object PromptTemplates {
    ...
    const val SCREEN_ANALYZE_SYSTEM_PROMPT = """
        ...
        2. [목적지 목록 선택 가이드] 목적지 검색 후 화면에 장소 목록(결과 리스트)이 나타나면, 절대 장소를 하나씩 물어보거나 임의로 선택(CLICK)하지 마세요. 대신, 리스트 영역 전체를 감싸는 컨테이너의 candidateId를 지정하고, actionType = "GUIDE"로 반환하세요. ttsMessage는 어르신이 목록 중에서 직접 고를 수 있게 가이드하는 말을 상황과 앱에 맞춰 자연스럽게 제공하세요.
        ...
        - 장소 검색 결과 화면에서 장소 목록이 있는 경우, 장소 목록 가이드(actionType: GUIDE)를 통해 사용자가 원하는 장소를 직접 고를 수 있게 하세요.
